...
// Command type constants for Tuya protocol
const CMD = {
    BIND: 3,           // Key exchange step 1 (send local key)
    RENAME_GW: 4,      // Key exchange step 2 (receive remote key)
    RENAME_DEVICE: 5,  // Key exchange step 3 (confirm)
    CONTROL: 7,        // Send command (v3.1-3.3)
    STATUS: 8,         // Status response
    HEART_BEAT: 9,     // Ping/pong
    DP_QUERY: 10,      // Query device state (v3.1-3.3)
    CONTROL_NEW: 13,   // Send command (v3.4+)
    DP_QUERY_NEW: 16,  // Query device state (v3.4+)
    SCENE_EXECUTE: 18  // Scene execution
};

// Packet frame markers
const PREFIX_55AA = 0x000055aa; // v3.1-3.4 packet prefix
const PREFIX_6699 = 0x00006699; // v3.5 packet prefix
const SUFFIX_AA55 = 0x0000aa55; // v3.1-3.4 packet suffix
const SUFFIX_9966 = 0x00009966; // v3.5 packet suffix
...
class TuyaAccessory extends EventEmitter {
    constructor(props) {
        ...
        const versionNum = parseFloat(this.context.version);
        this._msgQueue = async.queue(this[
            versionNum < 3.2 ? '_msgHandler_3_1' :
            versionNum < 3.4 ? '_msgHandler_3_3' :
            versionNum < 3.5 ? '_msgHandler_3_4' : '_msgHandler_3_5'
        ].bind(this), 1);
    ...
    update(o) {
        ...
        if (hasDataPoint) {
            ...
            if (this.context.sendEmptyUpdate) {
                ...
                this._send({cmd: (versionNum >= 3.4) ? 13 : 7});
            }
        } else {
    ...
    _send(o) {
        ...
        const versionNum = parseFloat(this.context.version);
        if (versionNum < 3.2) return this._send_3_1(o);
        if (versionNum < 3.4) return this._send_3_3(o);
        if (versionNum < 3.5) return this._send_3_4(o);
...
const encrypt35 = (data, encryptKey, iv) => {
    ...
    // Empty AAD for session key derivation (not message encryption)
    cipher.setAAD(Buffer.alloc(0));
    const encrypted = Buffer.concat([cipher.update(data), cipher.final()]);
    /* auth tag is cipher.getAuthTag() but not required here for key derivation */
    return encrypted;
};

const hmac = (data, hmacKey) => {
    // Don't specify encoding - let Node handle Buffer vs string automatically
