...
class TuyaDiscovery extends EventEmitter {
    ...
    stop() {
        ...
        this._stop(6666);
        this._stop(6667);
        this._stop(7000); // v3.5 discovery port
    ...
    _onDiscover(data) {
        ...
        // Pre-3.5 devices include version in broadcast; if missing, assume 3.5 (which doesn't broadcast version)
    ...
    _sendV35Probe() {
        ...
        // Build 14-byte AAD header: unknown(2) + seq(4) + cmd(4) + len(4)
        const unknownBuf = Buffer.alloc(2); // 0x0000
        const seqBuf = Buffer.alloc(4);     // sequence = 0
        const cmdBuf = Buffer.alloc(4);     // command = 0 for discovery
        const lenBuf = Buffer.alloc(4);
        // len = iv(12) + encrypted_payload + tag(16)
        // We need to encrypt first to know the length
        const cipher = crypto.createCipheriv('aes-128-gcm', GCM_DISCOVERY_KEY, iv);
        const aad = Buffer.concat([unknownBuf, seqBuf, cmdBuf, Buffer.alloc(4)]); // temporary AAD with zeroed length
        cipher.setAAD(aad);
        const enc = Buffer.concat([cipher.update(payload), cipher.final()]);
        const tag = cipher.getAuthTag();
        
        // Now compute actual length and rebuild with correct AAD
        const lenValue = iv.length + enc.length + tag.length; // 12 + enc.length + 16
        lenBuf.writeUInt32BE(lenValue, 0);
        const aadFinal = Buffer.concat([unknownBuf, seqBuf, cmdBuf, lenBuf]);
        
        // Re-encrypt with correct AAD
        const iv2 = crypto.randomBytes(12);
        const cipher2 = crypto.createCipheriv('aes-128-gcm', GCM_DISCOVERY_KEY, iv2);
        cipher2.setAAD(aadFinal);
        const enc2 = Buffer.concat([cipher2.update(payload), cipher2.final()]);
        const tag2 = cipher2.getAuthTag();
        
        // Update length if payload size changed (shouldn't, but be safe)
        const lenValue2 = iv2.length + enc2.length + tag2.length;
        lenBuf.writeUInt32BE(lenValue2, 0);
        
        const frame = Buffer.concat([
            Buffer.from('00006699', 'hex'), // v3.5 prefix
            unknownBuf,
            seqBuf,
            cmdBuf,
            lenBuf,
            ...
            enc2,
            tag2,
            Buffer.from('00009966', 'hex') // v3.5 suffix
        ]);
        
        // Enable broadcast before sending to 255.255.255.255
        socket.bind(() => {
